// components/DestinationsListClient.js
...
// Dynamically import ConfirmDeleteToast to prevent SSR issues
const ConfirmDeleteToast = dynamic(
  () => import('@/components/ConfirmDeleteToast'),
  { ssr: false }
);

const DestinationsListClient = ({ initialDestinations }) => {
  ...
  const deleteDestination = async (destination) => {
    try {
      ...
      setDestinations((prev) => prev.filter((dest) => dest.id !== destination.id));
                ...
                <tr key={dest.id} className="hover:bg-gray-50">
                  ...
                  <td className="py-4 px-6">
                    {dest.images && dest.images.length > 0 ? (
                      <Image
                        src={dest.images[0].url || dest.images[0]}
                        alt={dest.title}
                        width={80}
                        height={60}
...
export default DestinationsListClient;