...
export default function TourCard({ tour }) {
  ...
  // Function to get cost from season data
  const getPriceFromSeason = (seasonData) => {
    if (seasonData && seasonData.costs && seasonData.costs.cost) {
      return seasonData.costs.cost || 'N/A';
    }
    return 'N/A';
  };
  ...
  if (currentSeason === 'highSeason') {
    price = getPriceFromSeason(highSeason);
  } else if (currentSeason === 'lowSeason') {
    price = getPriceFromSeason(lowSeason);
  } else if (currentSeason === 'midSeason') {
    price = getPriceFromSeason(midSeason);
  }

  console.log('Price for tour:', price);

  return (
    <Link href={`/tours/${slug}`} className="tourCard block border bg-white hover:shadow-lg overflow-hidden rounded">
