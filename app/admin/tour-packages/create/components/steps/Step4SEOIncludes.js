...
// Utility function to remove duplicates based on the 'value' property
const removeDuplicates = (items) => {
  if (!Array.isArray(items)) {
    console.warn('removeDuplicates called with non-array:', items);
    return [];
  }
  const uniqueItems = [];
  ...
  items.forEach(item => {
    if (item && item.value && !seenValues.has(item.value)) {
...
const Step4SEOIncludes = ({ isEdit }) => {
  ...
  const handleGenerateSEO = async () => {
    if (!tourTitle || !description) {
      toast.error('Please fill in all required fields (Tour Title and Description) before generating SEO.');
      return;
    }

    if (!tags || tags.length === 0) {
      toast.error('Please add at least one tag before generating SEO.');
      return;
    }

    const isValid = await trigger(['basicInfo.tourTitle', 'basicInfo.description', 'basicInfo.tags']);
    if (!isValid) {
      toast.error('Please resolve the validation errors before generating SEO.');
      return;
    }
  ...
  // Initialize SEO fields and Includes/Excludes from existing data in edit mode
  useEffect(() => {
    if (isEdit) {
      // Initialize SEO Fields
      const existingSEO = getValues('seo');
      console.log('Existing SEO Data:', existingSEO);
      if (existingSEO) {
      ...
      // Initialize Includes
      const existingIncludes = getValues('includes');
      console.log('Existing Includes:', existingIncludes);
      if (existingIncludes && Array.isArray(existingIncludes)) {
        // Remove duplicates before setting
        const uniqueIncludes = removeDuplicates(existingIncludes);
        const formattedIncludes = uniqueIncludes.map(item => {
          // If items are strings, convert them to { value, label } objects
        ...
        console.log('Formatted Includes:', formattedIncludes);
        setValue('includes', formattedIncludes, { shouldValidate: true, shouldDirty: true });
      } else {
        // If includes is not an array, reset to empty array
        setValue('includes', [], { shouldValidate: true, shouldDirty: true });
      }

      // Initialize Excludes
      const existingExcludes = getValues('excludes');
      console.log('Existing Excludes:', existingExcludes);
      if (existingExcludes && Array.isArray(existingExcludes)) {
        // Remove duplicates before setting
        const uniqueExcludes = removeDuplicates(existingExcludes);
        const formattedExcludes = uniqueExcludes.map(item => {
          // If items are strings, convert them to { value, label } objects
          if (typeof item === 'string') {
            return { value: item, label: item };
          }
        ...
        console.log('Formatted Excludes:', formattedExcludes);
        setValue('excludes', formattedExcludes, { shouldValidate: true, shouldDirty: true });
      } else {
        // If excludes is not an array, reset to empty array
        setValue('excludes', [], { shouldValidate: true, shouldDirty: true });
      }
  ...
  const onSubmit = async (e) => {
    ...
    const uniqueIncludes = removeDuplicates(includesData);
    const uniqueExcludes = removeDuplicates(excludesData);

    // Log data for debugging
    console.log('Submitting SEO Data:', seoData);
    console.log('Submitting Includes:', uniqueIncludes);
    console.log('Submitting Excludes:', uniqueExcludes);

    // Update form data in context
    updateFormData('seo', seoData);
    updateFormData('includes', uniqueIncludes);
    updateFormData('excludes', uniqueExcludes);
  ...
  return (
    <form onSubmit={onSubmit} className="max-w-4xl mx-auto p-4">
      ...
      <div className="mb-6">
          ...
          render={({ field }) => (
            <CreatableSelect
              ...
              onChange={(value) => {
                ...
                const uniqueValues = removeDuplicates(value);
        ...
        />
        {errors.includes && (
          <p className="text-red-500">{errors.includes.message}</p>
        )}
      </div>
      ...
      <div className="mb-6">
        ...
        />
        {errors.excludes && (
          <p className="text-red-500">{errors.excludes.message}</p>
        )}
      </div>
      ...
      <div className="mb-4">
        ...
        <input
          id="seoTitle_en"
          {...register('seo.seoTitle_en', { required: 'SEO Title is required' })}
        ...
        <div className="mb-4">
          ...
          <input
            ...
            {...register('seo.seoTitle_DE', {
              required: 'SEO Title (German) is required',
            })}
            className={`p-2 border rounded w-full ${
        ...
        <div className="mb-4">
          ...
          <textarea
            ...
            {...register('seo.seoDescription_DE', {
              required: 'SEO Description (German) is required',
            })}
            className={`p-2 border rounded w-full ${
        ...
        <div className="mb-4">
          ...
          <input
            ...
            {...register('seo.seoKeywords_DE', {
              required: 'SEO Keywords (German) are required',
            })}
            className={`p-2 border rounded w-full ${
      ...
      <div className="flex justify-between mt-6">
        ...
        <button
          ...
          className={`bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700 transition-colors ${
            !seoGenerated ? 'opacity-50 cursor-not-allowed' : ''
          }`}
          disabled={!seoGenerated}
        >
          Next
        </button>
      </div>
