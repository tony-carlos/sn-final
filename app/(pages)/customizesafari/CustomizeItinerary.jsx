...
export default function CustomizeItinerary() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('/api/submitItinerary', payload);

      // Check if the response status is within the 2xx range
      if (response.status >= 200 && response.status < 300) {
        toast.success('Your itinerary has been submitted successfully!');
        ...
        throw new Error('Non-success status code received');
      }
    } catch (error) {
      console.error('Error submitting form:', error);
      toast.error('There was an error submitting your itinerary.');
    } finally {
  ...
  return (
    <section className="customize-itinerary" style={{ paddingBottom: '50px' }}>
      <ToastContainer position="top-right" autoClose={3000} />

      <Container
        maxWidth="md"
        sx={{ mt: { xs: 4, sm: 6, md: 8 }, mb: { xs: 4, sm: 6, md: 8 } }}
      >
        <Box
          component="form"
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} sm={6} md={4}>
              <Autocomplete
                ...
                renderInput={(params) => (
                  <TextField {...params} label="Choose a type of trip" required />
                )}
                fullWidth
              />
            </Grid>
            ...
            <Grid item xs={12}>
              <FormControlLabel
                ...
                label="I agree to receive a customized travel proposal and relevant updates about my itinerary."
              />
            </Grid>
